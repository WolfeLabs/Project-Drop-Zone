...
if (CASH < ( _healCost * (missionNamespace getVariable ["mav_ttm_var_CanadaHealthInsurance", 1]))) exitWith {hint format [localize "STR_NOTF_HS_NoCash",[_healCost] call life_fnc_numberText];};
...
if (_action) then {
    ...
    player setDamage 0;
    CASH = CASH - ( _healCost * (missionNamespace getVariable ["mav_ttm_var_CanadaHealthInsurance", 1]));
